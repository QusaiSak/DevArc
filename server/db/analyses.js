...
async function getAnalysis(userId, projectId) {
  ...
  return {
    ...
    codeAnalysis: analysis.codeAnalysis ? JSON.parse(analysis.codeAnalysis) : null,
    documentation: analysis.documentation ? JSON.parse(analysis.documentation) : null,
    testCases: analysis.testCases ? JSON.parse(analysis.testCases) : null,
  };
}

async function getAllUserAnalyses(userId) {
  const userAnalyses = await db.select().from(analyses)
