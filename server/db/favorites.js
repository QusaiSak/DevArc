...
async function addFavorite({ userId, repo }) {
  ...
  if (!repo || !repo.id || !repo.name || !repo.owner?.login) throw new Error('Invalid repo object');
...
async function isFavorite({ userId, repoId }) {
  const result = await db.select().from(favorites).where(and(eq(favorites.userId, userId), eq(favorites.repoId, repoId)));
