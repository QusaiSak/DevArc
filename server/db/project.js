...
async function createProject(projectData) {
  try {
    ...
    console.log(
      `✅ Project created: ${newProject.name} (ID: ${newProject.id})`
    );
    return newProject;
  } catch (error) {
...
async function updateProject(projectId, userId, updateData) {
  try {
    ...
    const { name, description, sdlc, questions, tags, visibility } = updateData;
    ...
    const [updatedProject] = await db
      ...
      .set({
        ...
        questions:
          questions !== undefined ? questions : existingProject.questions,
        tags: tags !== undefined ? tags : existingProject.tags,
        visibility:
          visibility !== undefined ? visibility : existingProject.visibility,
        updatedAt: new Date(),
      })
    ...
    console.log(
      `✅ Project updated: ${updatedProject.name} (ID: ${updatedProject.id})`
    );
...
async function deleteProject(projectId, userId) {
  try {
    ...
    console.log(
      `✅ Project deleted: ${existingProject.name} (ID: ${existingProject.id})`
    );
    return true;
  } catch (error) {
...
async function getProjectsByVisibility(userId, visibility) {
  try {
    const filteredProjects = await db
      ...
      .from(projects)
      .where(
        and(eq(projects.userId, userId), eq(projects.visibility, visibility))
      )
      .orderBy(desc(projects.createdAt));
...
async function getProjectStats(userId) {
  try {
    ...
    const privateProjects = await db
      .select({
        count: count(),
      })
      .from(projects)
      .where(
        and(eq(projects.userId, userId), eq(projects.visibility, "private"))
      );
