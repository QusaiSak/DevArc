  ...
  const loadProject = async (projectId: number) => {
    try {
      ...
      if (projectData.repoUrl) {
        try {
          const repoPath = projectData.repoUrl.replace('https://github.com/', '')
      ...
      <div className="min-h-screen bg-background flex items-center justify-center">
        <Card className="max-w-md border-0 shadow-lg bg-card/80 backdrop-blur-xl">
          <CardContent className="pt-8 text-center">
            ...
            <p className="text-destructive mb-6">{error || 'Project not found'}</p>
            <Button 
            ...
            <div className="flex flex-col lg:flex-row lg:items-start lg:justify-between gap-6">
              <div className="space-y-4 flex-1">
                <div className="flex items-center gap-3">
                  ...
                  <Badge variant="secondary" className="capitalize font-medium shadow-sm">
                    {project.visibility}
                  </Badge>
                </div>
                ...
                {project.sdlc?.phases && (
                  <div className="bg-gradient-to-br from-muted/30 to-muted/10 rounded-xl p-5 border border-border/50 shadow-inner">
                    ...
                    <div className="grid gap-3">
                      {project.sdlc.phases.map((phase: string, index: number) => (
                        <div 
                          key={index} 
                    ...
                    <div className="bg-gradient-to-r from-muted/80 to-muted/60 px-6 py-4 border-b border-border/50 flex items-center justify-between backdrop-blur-sm">
                      <div className="flex items-center gap-4">
                        ...
                        <div className="flex items-center gap-2">
                          ...
                          <span className="text-sm font-mono font-medium">README.md</span>
                        </div>
                      </div>
                      <Badge variant="secondary" className="shadow-sm">Documentation</Badge>
                    </div>
                      ...
                      <div className="p-8">
                        <div className="prose prose-slate dark:prose-invert max-w-none prose-lg">
                          <ReactMarkdown 
                            ...
                            components={{
                              h1: ({...props}) => (
                                <h1 className="text-3xl font-bold mb-6 pb-4 border-b border-border/50 text-foreground" {...props} />
                              ),
                              h2: ({...props}) => (
                              ...
                              ul: ({...props}) => (
                                <ul className="list-disc list-inside mb-4 space-y-2 pl-4 text-muted-foreground" {...props} />
                              ),
                              ol: ({...props}) => (
                                <ol className="list-decimal list-inside mb-4 space-y-2 pl-4 text-muted-foreground" {...props} />
                              ),
                              li: ({...props}) => (
                              ...
                              code: (props: any) => {
                                ...
                                return isInline ? (
                                  <code className="bg-muted/80 px-2 py-1 rounded-md text-sm font-mono text-foreground border border-border/30" {...rest}>
                                    {children}
                                  </code>
                                ) : (
                                  <code className="block bg-muted/60 p-4 rounded-lg text-sm font-mono overflow-x-auto text-foreground border border-border/30" {...rest}>
                                    {children}
                                  </code>
                                );
                              },
                              pre: ({children, ...props}) => (
                                <pre className="bg-muted/60 p-6 rounded-xl overflow-x-auto text-sm font-mono mb-6 border border-border/30 shadow-inner" {...props}>
                                  {children}
                                </pre>
                              ),
                              a: ({...props}) => (
                                <a className="text-primary hover:text-primary/80 hover:underline font-medium transition-colors" {...props} />
                              ),
                              blockquote: ({...props}) => (
                                <blockquote className="border-l-4 border-primary/50 pl-6 py-4 my-6 bg-muted/30 rounded-r-xl italic text-muted-foreground" {...props} />
                              ),
                              table: ({...props}) => (
                              ...
                              thead: ({...props}) => (
                                <thead className="bg-gradient-to-r from-muted/60 to-muted/40" {...props} />
                              ),
                              tbody: ({...props}) => (
                                <tbody className="divide-y divide-border/30" {...props} />
                              ),
                              th: ({...props}) => (
                                <th className="bg-muted/40 border-r border-border/30 last:border-r-0 px-6 py-4 text-left font-semibold text-sm text-foreground" {...props} />
                              ),
                              td: ({...props}) => (
                                <td className="border-r border-border/20 last:border-r-0 px-6 py-4 text-sm text-muted-foreground" {...props} />
                              ),
                              hr: ({...props}) => (
                                <hr className="my-8 border-border/50" {...props} />
                              ),
                              img: ({...props}) => (
                                <img className="max-w-full h-auto rounded-xl shadow-lg my-6 border border-border/30" {...props} />
                              ),
                            }}
                          >
                            {readmeContent}
                  ...
                  <div className="p-12 text-center">
                    ...
                    {project.repoUrl ? (
                      <div className="space-y-4 max-w-md mx-auto">
                        <p className="text-muted-foreground leading-relaxed">
                          The repository might not have a README.md file or it could not be loaded.
                        </p>
                        <Button 
                          variant="outline"
                          onClick={() => window.open(project.repoUrl, '_blank')}
                      ...
                      <p className="text-muted-foreground max-w-md mx-auto leading-relaxed">
                        This project doesn't have an associated GitHub repository.
                      </p>
                    )}
                  </div>
                )}
              ...
              <CardContent className="p-5 space-y-4">
                <div className="flex items-center gap-4 p-4 bg-gradient-to-br from-muted/40 to-muted/20 rounded-xl border border-border/30 shadow-sm">
                  ...
                  <div className="flex-1">
                    <p className="text-xs text-muted-foreground uppercase tracking-wider font-medium mb-1">Created</p>
                ...
                <div className="flex items-center gap-4 p-4 bg-gradient-to-br from-muted/40 to-muted/20 rounded-xl border border-border/30 shadow-sm">
                  ...
                  <div className="flex-1">
                    <p className="text-xs text-muted-foreground uppercase tracking-wider font-medium mb-1">Visibility</p>
              ...
              <CardContent className="p-5">
                ...
                <div className="p-4 bg-gradient-to-br from-muted/40 to-muted/20 rounded-xl border border-border/30 shadow-sm">
                  <div className="flex items-center justify-between">
                    <span className="text-xs text-muted-foreground uppercase tracking-wider font-medium">Repository</span>
                    <div className="flex items-center gap-3">
                      <div className="relative">
                        <div className={`w-2.5 h-2.5 rounded-full ${project.repoUrl ? 'bg-green-500' : 'bg-gray-500'} shadow-sm`}></div>
                        {project.repoUrl && (
