...
export class StructureAnalyzer {
    ...
    try {
      ...
      for (let i = 0; i < codeFiles.length; i += batchSize) {
        ...
        const batchPromises = batch.map(async (file) => {
          try {
            ...
            if (content && content.length > 0) {
              ...
              console.log(
                `Successfully parsed ${file.path}: ${parsed.lines} lines, ${parsed.functions.length} functions`
              );
              return parsed;
            }
        ...
        const validResults = batchResults.filter(
          (result): result is ParsedFile => result !== null
        );
  ...
  private buildProjectStructure(
    ...
    const testCoverage =
      totalFiles > 0 ? (testFiles.length / totalFiles) * 100 : 0;
  ...
  private classifyDirectory(path: string): DirectoryInfo["type"] {
    ...
    if (lowerPath.includes("src") || lowerPath.includes("source"))
      return "source";
    if (
  ...
  private detectArchitecturePatterns(
    ...
    const packageJsonContent = parsedFiles.find((f) =>
    ...
    if (
      fileNames.some(
        (name) => name.endsWith(".jsx") || name.endsWith(".tsx")
      ) ||
    ...
    // Vue
    if (
      fileNames.some((name) => name.endsWith(".vue")) ||
    ...
    if (
      fileNames.some(
        (name) => name.includes("angular") || name.endsWith(".component.ts")
      ) ||
    ...
    if (
      hasPackageJson ||
      filePaths.some((path) => path.includes("node_modules"))
    ) {
    ...
    if (
      ...
      parsedFiles.some((f) =>
        f.imports.some((imp) => imp.module.includes("springframework"))
      )
    ) {
    ...
    // Flask (Python)
    if (
      parsedFiles.some((f) => f.imports.some((imp) => imp.module === "flask"))
    ) {
    ...
    if (
      filePaths.some(
        (path) => path.includes("components") && path.includes("pages")
      )
    ) {
    ...
    if (
      filePaths.some(
        (path) => path.includes("services") && path.includes("components")
      )
    ) {
    ...
    if (
      filePaths.some((path) => path.includes("api") || path.includes("routes"))
    ) {
