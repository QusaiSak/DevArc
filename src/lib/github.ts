import type { GitHubFile, Repository, GitHubCreateRepoResponse, GitHubCommit, GitHubSearchResponse } from '@/types/github.interface'
...
export class GitHubService {
  ...
  }  async getUserRepositories(username: string, page = 1, perPage = 30): Promise<Repository[]> {
    try {
  ...
  }  async getAuthenticatedUserRepositories(page = 1, perPage = 30): Promise<Repository[]> {
    try {
  ...
  async getRepositories(username?: string): Promise<Repository[]> {
    try {
      ...
      while (hasMore && currentPage <= 10) { // Limit to 10 pages (1000 repos max) to avoid infinite loops
        ...
        if (username) {
          pageRepos = await this.getUserRepositories(username, currentPage, 100)
        } else {
          pageRepos = await this.getAuthenticatedUserRepositories(currentPage, 100)
        }
  ...
  async searchRepositories(query: string, page = 1, perPage = 30): Promise<GitHubSearchResponse<Repository>> {
    try {
   ...
   */
  async createRepository(name: string, description = '', isPrivate = false): Promise<GitHubCreateRepoResponse> {
    try {
      ...
      if (error instanceof Error && error.message.includes('name already exists')) {
        ...
        console.log(`🔄 Repository name exists, trying with timestamp: ${name}-${timestamp}`);
        return this.createRepository(`${name}-${timestamp}`, description, isPrivate);
      }
  ...
  async pushFile(
    ...
    try {
      ...
      console.log(`✅ File "${path}" pushed to "${repo}" on branch "${branch}"`)
      return data
    } catch (error) {
